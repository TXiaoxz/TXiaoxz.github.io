...
import FrameBufferObject from './fbo.js'
...
class RenderPasses extends WebGlApp 
{
    ...
    renderpass_pixel_filter( gl, canvas_width, canvas_height )
    {
        ...
        // ...
        this.quad.render(gl, this.filter_mode, fbo.getColorTexture(), fbo.getDepthTexture());
        // render only lights
    ...
    do_depth_pass( gl, fbo, current_light )
    {
        ...
        let output_projection_matrix = mat4.multiply(mat4.create(), shadow_p, shadow_v);
        return output_projection_matrix; // TODO compute the output projection matrix
    }
    ...
    renderpass_shadowmap( gl, canvas_width, canvas_height )
    {
        ...
        this.fbo = this.fbo_directional;
        {
            ...
            this.scene.setShader(gl, this.shadow_shader)
            {
                ...
                shader.setUniform4x4f('u_view', viewMatrix);
                shader.setUniform4x4f('u_projection', projectionMatrix);
